...
class CustomerState {
  const CustomerState({
    this.customers = const [], this.filtered = const [],
    this.status = CustomerStatus.idle,
    this.searchQuery = '', this.errorMessage,
  });
  ...
  CustomerState copyWith({
    List<CustomerEntity>? customers, List<CustomerEntity>? filtered,
    CustomerStatus? status, String? searchQuery, String? errorMessage,
...
class CustomerViewModel extends Notifier<CustomerState> {
    ...
    final sub = watchUC(const NoParams()).listen((either) {
      either.fold(
        ...
        (list) => state = state.copyWith(
            customers: list, filtered: _filter(list, state.searchQuery)),
      );
    });
  ...
  void search(String q) => state = state.copyWith(
      searchQuery: q, filtered: _filter(state.customers, q));
  ...
  Future<bool> addCustomer(CustomerParams params) async {
    ...
    return result.fold(
      (f) { state = state.copyWith(status: CustomerStatus.error, errorMessage: f.message); return false; },
      (_) { state = state.copyWith(status: CustomerStatus.success); return true; },
    );
  }
  ...
  Future<bool> deleteCustomer(String id) async {
    ...
    return result.fold(
      (f) { state = state.copyWith(errorMessage: f.message); return false; },
      (_) => true,
    );
  }
}
...
final customerViewModelProvider =
    NotifierProvider<CustomerViewModel, CustomerState>(CustomerViewModel.new);

// Watch use case provider (used in ViewModel)
